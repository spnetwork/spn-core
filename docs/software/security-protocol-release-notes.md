...
* have a summary view of changes that affect the code base (spn-core has to be able to replay all ledgers generated since genesis on the Stellar public network).
...
* this document is not where security disclosures are made, instead follow the process outlined in [Stellar's bug bounty program](https://www.spn.org/bug-bounty-program/) as a way to triage and respond to issues.
