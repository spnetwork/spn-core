...
This document describes various aspects of running `spn-core` for **system administrators** (but may be useful to a broader audience).
...
It will also be useful to understand how [data flows](https://www.spn.org/developers/spn-core/software/core-data-flow.pdf) and is stored in the system.

## Zero to completed: node checklist
 - [ ] [deciding to run a node](#why-run-a-node)
 - [ ] [setting up an instance to run core](#instance-setup)
 - [ ] [install spn-core](#installing)
...
note: in this document we use "Horizon" as the example implementation of a first tier service built on top of spn-core, but any other system would get the same benefits.
...
## Instance setup
Regardless of how you install spn-core (apt, source, docker, etc), you will need to configure the instance hosting it roughly the same way.
...
As of early 2018, spn-core with PostgreSQL running on the same machine has no problem running on a [m5.large](https://aws.amazon.com/ec2/instance-types/m5/) in AWS (dual core 2.5 GHz Intel Xeon, 8 GB RAM).
...
* **inbound**: spn-core needs to allow all ips to connect to its `PEER_PORT` (default 11625) over TCP.
* **outbound**: spn-core needs access to connect to other peers on the internet on `PEER_PORT` (most use the default as well) over TCP.

#### Interaction with other internal systems

* **outbound**:
  * spn-core needs access to a database (postgresql for example), which may reside on a different machine on the network
  * other connections can safely be blocked
* **inbound**: spn-core exposes an *unauthenticated* HTTP endpoint on port `HTTP_PORT` (default 11626)
...
In general you should aim to run the latest [release](https://github.com/spn/spn-core/releases) as builds are backward compatible and are cummulative.
...
See the [INSTALL](https://github.com/spn/spn-core/blob/master/INSTALL.md) for build instructions.

### Package based Installation
If you are using Ubuntu 16.04 LTS we provide the latest stable releases of [spn-core](https://github.com/spn/spn-core) and [spn-horizon](https://github.com/spn/go/tree/master/services/horizon) in Debian binary package format.

See [detailed installation instructions](https://github.com/spn/packages#sdf---packages)

### Container based installation
Docker images are maintained in a few places, good starting points are:
 * the [quickstart image](https://github.com/spn/docker-spn-core-horizon)
 * the [standalone image](https://github.com/spn/docker-spn-core). **Warning**: this only tracks the latest master, so you have to find the image based on the [release](https://github.com/spn/spn-core/releases) that you want to use.

## Configuring

Before attempting to configure spn-core, it is highly recommended to first try running a private network or joining the test network. 

### Configuration basics
All configuration for spn-core is done with a TOML file. By default 
spn-core loads `./spn-core.cfg`, but you can specify a different file to load on the command line:

`$ spn-core <COMMAND> --conf betterfile.cfg` 

The [example config](https://github.com/spn/spn-core/blob/master/docs/spn-core_example.cfg) is not a real configuration, but documents all possible configuration elements as well as their default values.

Here is an [example test network config](https://github.com/spn/docker-spn-core-horizon/blob/master/testnet/core/etc/spn-core.cfg) for connecting to the test network.

Here is an [example public network config](https://github.com/spn/docs/blob/master/other/spn-core-validator-example.cfg) for connecting to the public network.
...
This section describes how to configure the quorum set for a validator and assumes basic understanding of the [Stellar Consensus Protocol](https://www.spn.org/developers/guides/concepts/scp.html).

#### Validator list

You will find lists of validators in a few places:
* [list of validators](https://github.com/spn/docs/blob/master/validators.md)
* the [Stellar Dashboard](https://dashboard.spn.org/)

#### Understanding requirements for a good quorum

The way quorum sets are configured is explained in detail in the [example config](https://github.com/spn/spn-core/blob/master/docs/spn-core_example.cfg).
...
* [Automatic maintenance](#cursors-and-automatic-maintenance) configured properly, especially when spn-core is used in conjunction with a downstream system like Horizon.

### Database and local state

After configuring your [database](#database) and [buckets](#buckets) settings, when running spn-core for the first time, you must initialize the database:
...
see the [example config](https://github.com/spn/spn-core/blob/master/docs/spn-core_example.cfg).
...
By default, spn-core will perform this automatic maintenance, so be sure to disable it until you have done the appropriate data ingestion in downstream systems (Horizon for example sometimes needs to reingest data).
...
Buckets should be stored on a fast local disk with sufficient space to store several times the size of the current ledger. 
 ...
 For the most part, the contents of both directories can be ignored as they are managed by spn-core.
...
The [example config](https://github.com/spn/spn-core/blob/master/docs/spn-core_example.cfg) 
shows how to configure a history archive through command templates. 

While it is possible to run a spn-core node with no configured history 
...
`$ spn-core new-hist <historyarchive>`
...
After having configured your node and its environment, you're ready to start spn-core.
...
Review the [logging](#logging) section to get yourself familiar with the output of spn-core.

### Interacting with your instance
While running, interaction with spn-core is done via an administrative 
HTTP endpoint. Commands can be submitted using command-line HTTP tools such 
as `curl`, or by running a command such as

`$ spn-core http-command <http-command>`
...
## Logging
Stellar-core sends logs to standard output and `spn-core.log` by default, 
...
`$ spn-core http-command "ll?level=debug"`
...
`$ spn-core http-command "ll?level=debug&partition=history"` 
...
Run `$ spn-core http-command 'info'`
The output will look something like
```json
 {
   "info" : {
...
* `build` is the build number for this spn-core instance
...
`$ spn-core http-command 'quorum?node=$sdf1` or `$ spn-core http-command 'quorum?node=@GABCDE` 
...
1. `$ spn-core http-command 'upgrades?mode=set&upgradetime=2018-01-31T20:00:00Z&protocolversion=9'`

2. `$ spn-core http-command info`
...
example, see [docker-spn-core](https://github.com/spn/docker-spn-core-horizon).

### In depth architecture

[architecture.md](https://github.com/spn/spn-core/blob/master/docs/architecture.md) 
  describes how spn-core is structured internally, how it is intended to be 
