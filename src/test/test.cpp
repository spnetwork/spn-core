...
getTestConfig(int instanceNumber, Config::TestDbMode mode)
{
    ...
    {
        gTestRoots.emplace_back("spn-core-test");
        ...
        sstream << "spn" << instanceNumber << ".log";
     ...
     std::vector<std::string> const& metrics)
{
    ...
    // not mixed with spn-core logging.
    Logging::setFmt("<test>");
    ...
    LOG(INFO) << "Testing spn-core " << STELLAR_CORE_VERSION;
    ...
    cli |= clara::Opt(gBaseInstance, "offset")["--base-instance"](
        ...
        "spn-core can run tests concurrently");
    session.cli(cli);
