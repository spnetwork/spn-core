...
{
    auto res = spn::loadAccountWithoutRecord(ls, k);
    if (mustExist)
    {
        REQUIRE(res);
    }
...
{
    LedgerState ls(app.getLedgerStateRoot());
    return (bool)spn::loadAccountWithoutRecord(ls, k);
}
...
{
    LedgerState ls(app.getLedgerStateRoot());
    auto account = spn::loadAccount(ls, k);
                       ...
                       SequenceNumber seq)
{
    ...
    case MANAGE_OFFER_UPDATED:
    {
        LedgerState ls(app.getLedgerStateRoot());
        auto offer =
            spn::loadOffer(ls, source.getPublicKey(), expectedOfferID);
        REQUIRE(offer);
                        ...
                        ManageOfferEffect expectedEffect)
{
    ...
    if (createPassiveOfferResult.code() == MANAGE_OFFER_SUCCESS)
    {
        ...
        {
            LedgerState ls(app.getLedgerStateRoot());
            REQUIRE(!spn::loadOffer(ls, source.getPublicKey(),
                                        expectedOfferID));
        }
        break;
        default:
