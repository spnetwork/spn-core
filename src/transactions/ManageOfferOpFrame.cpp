                                    ...
                                    AbstractLedgerState& lsOuter)
{
    ...
    if (sheep.type() != ASSET_TYPE_NATIVE)
    {
        ...
        auto issuer = spn::loadAccount(ls, getIssuer(sheep));
        if (!issuer)
        {
            metrics
    ...
    if (wheat.type() != ASSET_TYPE_NATIVE)
    {
        ...
        auto issuer = spn::loadAccount(ls, getIssuer(wheat));
        if (!issuer)
        {
            metrics
...
ManageOfferOpFrame::doApply(Application& app, AbstractLedgerState& lsOuter)
{
    ...
    { // modifying an old offer
        auto sellSheepOffer = spn::loadOffer(ls, getSourceID(), offerID);
        if (!sellSheepOffer)
        {
            app.getMetrics()
    ...
    if (mManageOffer.amount > 0)
    {
        ...
        if (header.current().ledgerVersion >= 10)
        {
            ...
            {
                auto sourceAccount =
                    spn::loadAccountWithoutRecord(ls, getSourceID());
