                   ...
                   std::function<int(int)> getVote)
{
    ...
    for (int i = 0; i < nbAccounts; i++)
    {
        ...
        if (bal >= 0)
        {
            ...
            auto account = spn::loadAccount(ls, to.getPublicKey());
                  ...
                  std::function<int(int)> getVote, Application& app)
{
    ...
    for (auto w : winners)
    {
        ...
        if (ledgerVersion >= 10)
        {
            ...
            auto winner =
                spn::loadAccount(ls, getTestAccount(w).getPublicKey());
            toDoleToThis =
            ...
            std::function<int(int)> getVote, int expectedWinnerCount)
{
    ...
    for (int i = 0; i < nbAccounts; i++)
    {
        ...
        else
        {
            LedgerState ls(app.getLedgerStateRoot());
            auto account =
                spn::loadAccount(ls, getTestAccount(i).getPublicKey());
    ...
    for (int i = 0; i < nbAccounts; i++)
    {
        ...
        else
        {
            {
                LedgerState ls(app.getLedgerStateRoot());
                auto account = spn::loadAccount(ls, k.getPublicKey());
                auto const& ae = account.current().data.account();
