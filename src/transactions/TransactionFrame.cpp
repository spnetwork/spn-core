                              ...
                              AccountID const& accountID)
{
    ...
    {
        // this is buggy caching that existed in old versions of the protocol
        auto res = spn::loadAccount(ls, accountID);
        if (res)
        {
    ...
    {
        return spn::loadAccount(ls, accountID);
    }
}
...
TransactionFrame::processFeeSeqNum(AbstractLedgerState& ls)
{
    ...
    if (fee > 0)
    {
        ...
        spn::addBalance(acc.balance, -fee);
...
{
    auto account = spn::loadAccount(ls, accountID);
    if (!account)
    {
                                      ...
                                      SignerKey const& signerKey) const
{
    ...
    {
        auto removed = spn::addNumEntries(header, account, -1);
        assert(removed);
