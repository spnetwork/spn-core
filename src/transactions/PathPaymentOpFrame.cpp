...
PathPaymentOpFrame::doApply(Application& app, AbstractLedgerState& ls)
{
    ...
    if (ls.loadHeader().current().ledgerVersion < 8)
    {
        doesSourceAccountExist =
            (bool)spn::loadAccountWithoutRecord(ls, getSourceID());
    }

    if (!bypassIssuerCheck)
    {
        if (!spn::loadAccountWithoutRecord(ls, mPathPayment.destination))
        {
            app.getMetrics()
    ...
    {
        auto destination = spn::loadAccount(ls, mPathPayment.destination);
    ...
    else
    {
        ...
        {
            auto issuer =
                spn::loadAccountWithoutRecord(ls, getIssuer(curB));
            if (!issuer)
            {
                app.getMetrics()
        ...
        auto destLine =
            spn::loadTrustLine(ls, mPathPayment.destination, curB);
        if (!destLine)
        {
            app.getMetrics()
    ...
    for (int i = (int)fullPath.size() - 1; i >= 0; i--)
    {
        ...
        {
            if (!spn::loadAccountWithoutRecord(ls, getIssuer(curA)))
            {
                app.getMetrics()
                    .NewMeter({"op-path-payment", "failure", "no-issuer"},
                              "operation")
    ...
    if (curB.type() == ASSET_TYPE_NATIVE)
    {
        ...
        {
            sourceAccount = spn::loadAccount(ls, getSourceID());
            if (!sourceAccount)
            {
                app.getMetrics()
