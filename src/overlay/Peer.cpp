...
void
Peer::recvMessage(StellarMessage const& spnMsg)
{
    if (shouldAbort())
    {
        return;
    ...
    if (Logging::logTrace("Overlay"))
        CLOG(TRACE, "Overlay")
            ...
            << mApp.getConfig().toShortString(
                   mApp.getConfig().NODE_SEED.getPublicKey())
            << ") recv: " << msgSummary(spnMsg)
            << " from:" << mApp.getConfig().toShortString(mPeerID);

    if (!isAuthenticated() && (spnMsg.type() != HELLO) &&
        (spnMsg.type() != AUTH) && (spnMsg.type() != ERROR_MSG))
    {
        CLOG(WARNING, "Overlay")
            << "recv: " << spnMsg.type() << " before completed handshake";
        drop();
        return;
    }

    assert(isAuthenticated() || spnMsg.type() == HELLO ||
           spnMsg.type() == AUTH || spnMsg.type() == ERROR_MSG);
