...
CommandHandler::testAcc(std::string const& params, std::string& retStr)
{
    ...
    else
    {
        ...
        auto acc = spn::loadAccount(ls, key.getPublicKey());
        if (acc)
        {
...
{
    retStr = "<b>Welcome to spn-core!</b><p>";
    ...
    retStr +=
        ...
        "Cursors are used by dependent services to tell spn - core which "
...
CommandHandler::manualClose(std::string const& params, std::string& retStr)
{
    ...
    {
        retStr =
            "Set MANUAL_CLOSE=true in the spn-core.cfg if you want this "
            "behavior";
    }
}
...
CommandHandler::generateLoad(std::string const& params, std::string& retStr)
{
    ...
    else
    {
        retStr = "Set ARTIFICIALLY_GENERATE_LOAD_FOR_TESTING=true in "
                 "the spn-core.cfg if you want this behavior";
    }
}
