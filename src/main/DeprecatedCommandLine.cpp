...
const struct option spn_core_options[] = {
...
usage(int err = 1)
{
    ...
    os << "usage: spn-core [OPTIONS]\n"
          ...
          "      --forcescp           Next time spn-core is run, SCP will "
          "start "
...
handleDeprecatedCommandLine(int argc, char* const* argv)
{
    ...
    std::string cfgFile("spn-core.cfg");
    std::string command;
    ...
    int opt;
    while ((opt = getopt_long_only(argc, argv, "c:", spn_core_options,
                                   nullptr)) != -1)
    {
        switch (opt)
        {
