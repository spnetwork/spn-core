...
CommandLine::ConfigOption::getConfig(bool logToFile) const
{
    Config config;
    auto configFile =
        mConfigFile.empty() ? std::string{"spn-core.cfg"} : mConfigFile;
...
runForceSCP(CommandLineArgs const& args)
{
    ...
    auto resetOption = clara::Opt{reset}["--reset"](
        "reset force SCP flag, so next time spn-core "
...
handleCommandLine(int argc, char* const* argv)
{
    auto commandLine = CommandLine{
        {{"catchup",
         ...
         {"force-scp",
          "next time spn-core is run, SCP will start with "
         ...
         {"http-command", "send a command to local spn-core",
          runHttpCommand},
